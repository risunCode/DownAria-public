 ...
 * Designed for export/import usage inside the current browser profile.
 * 
...
const BROWSER_KEY_STORAGE = 'downaria_browser_key';
const KEY_LENGTH = 32;

let cachedBrowserKey: string | null = null;
...
function generateBrowserKey(): string {
  ...
  return Array.from({ length: KEY_LENGTH }, () => Math.floor(Math.random() * 256).toString(16).padStart(2, '0')).join('');
}

function getBrowserStorageKey(): string {
  if (cachedBrowserKey) {
    return cachedBrowserKey;
  }

  if (typeof window === 'undefined') {
    return generateBrowserKey();
  }

  try {
    const storedKey = localStorage.getItem(BROWSER_KEY_STORAGE);
    if (storedKey && storedKey.trim()) {
      cachedBrowserKey = storedKey;
      return storedKey;
    }

    const generatedKey = generateBrowserKey();
    localStorage.setItem(BROWSER_KEY_STORAGE, generatedKey);
    cachedBrowserKey = generatedKey;
    return generatedKey;
  } catch {
    const fallbackKey = generateBrowserKey();
    cachedBrowserKey = fallbackKey;
    return fallbackKey;
  }
}
...
// ═══════════════════════════════════════════════════════════════
// ENCRYPTION KEY (Browser-local only)
// ═══════════════════════════════════════════════════════════════
...
function encryptValue(value: string): string {
  const browserKey = getBrowserStorageKey();
  const encrypted = xorCipher(value, browserKey);
  return `${ENCRYPTED_PREFIX}${encrypted}.${computeHMAC(encrypted, browserKey)}`;
}
...
function decryptValue(stored: string): string | null {
  ...
  if (hmac !== computeHMAC(encrypted, browserKey)) {
    return null;
  }

  return xorDecipher(encrypted, browserKey);
}
