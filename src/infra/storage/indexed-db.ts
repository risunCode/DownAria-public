...
export interface ScraperCacheEntry {
    key: string;        // Original URL
    platform: string;
    data: BackendResponse<BackendExtractData> | unknown;
    createdAt: number;
    expiresAt: number;
    lastAccess: number;
}
...
export async function importHistory(data: ExportData | string, merge = true): Promise<{ imported: number; skipped: number }> {
    ...
    for (const entry of exportData.history) {
        ...
        try {
            ...
            console.debug('[History] Failed to import entry', error);
            skipped++;
        }
    }
...
export async function setScraperCache(
    url: string,
    platform: string,
    data: BackendResponse<BackendExtractData> | unknown,
    ttlDays = 7
    ...
    return new Promise((resolve, reject) => {
        const tx = database.transaction(SCRAPER_CACHE_STORE, 'readwrite');
        const store = tx.objectStore(SCRAPER_CACHE_STORE);
...
export async function getScraperCache(url: string): Promise<BackendResponse<BackendExtractData> | unknown | null> {
    const database = await openDB();
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
            const entry = request.result as ScraperCacheEntry | undefined;
            if (!entry) {
                resolve(null);
                return;
            }
...
export async function clearScraperCache(): Promise<void> {
    const database = await openDB();
    return new Promise((resolve, reject) => {
        const tx = database.transaction(SCRAPER_CACHE_STORE, 'readwrite');
        const store = tx.objectStore(SCRAPER_CACHE_STORE);
...
export async function cleanupExpiredScraperCache(): Promise<number> {
    const database = await openDB();
    return new Promise((resolve, reject) => {
        const tx = database.transaction(SCRAPER_CACHE_STORE, 'readwrite');
        const store = tx.objectStore(SCRAPER_CACHE_STORE);
...
import { deleteBackgroundBlob, getBackgroundBlob, saveBackgroundBlob } from './seasonal.background';
