...
export function TrafficLights({ onClose, title }: TrafficLightsProps) {
  return (
    <div className="flex items-center gap-2 px-4 py-2.5 bg-[var(--bg-card)]/80 backdrop-blur-md border-b border-[var(--border-color)]/50">
      <button
        onClick={onClose}
      ...
      {title && (
        <span className="ml-2 text-xs text-[var(--text-muted)] font-medium">{title}</span>
      )}
    </div>
  );
}
