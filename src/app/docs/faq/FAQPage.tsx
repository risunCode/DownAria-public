...
const faqs: { category: string; items: FAQItem[] }[] = [
    ...
    },
    {
        ...
        items: [
            { question: 'Which endpoint should I start with?', answer: 'Start with `POST /api/v1/extract` to get media metadata and downloadable sources.' },
            { question: 'How do I download media files?', answer: 'Use `GET /api/v1/proxy` with the media URL from the extract result.' },
            { question: 'How do I use merge for YouTube downloads?', answer: 'Use `POST /api/v1/merge` with `{ url, quality?, format? }`. The backend resolves streams via yt-dlp and processes output with FFmpeg fast-path.' },
